...
double alpha = 0.5; //increment of dependance of y in terms of refractive index
...
double s = 0;
double ds = 1 ; // taille step incriment
double state[4] = {0.0, 1.0, 0.7071, -0.7071};
    ...
    for(int i = 0; i < 100000; i++) {
      ...
      //fflush(stdout);
        ...
        if( state[1] > 20) break; //si on est par terre ou trop haut
    }

    outFile.close();
    cout << state[3]*state[3] + state[2]*state[2] <<endl;
